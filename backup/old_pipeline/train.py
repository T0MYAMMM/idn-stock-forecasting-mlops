...
class StockForecaster:
    ...
    def save_best_model(self):
        """Save the best model for deployment with MLFlow Model Registry"""
        if self.best_model_info['type'] == 'advanced':
            ...
            try:
                with mlflow.start_run(run_name=f"Best_Model_Registration_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                    # Get feature columns that were actually used for training
                    target_cols = self.feature_metadata['target_columns']
                    feature_cols = [col for col in self.feature_metadata.keys() if col != 'target_columns']
                    all_features = []
                    for category in feature_cols:
                        all_features.extend(self.feature_metadata[category])
                    
                    # Create input example using the same feature preparation as training
                    input_example = None
                    signature = None
                    
                    try:
                        # Load test data and prepare features exactly like training
                        test_data = pd.read_csv('data/preprocessed/bbca_test.csv', 
                                              index_col='Date', parse_dates=True)
                        
                        # Use the exact same feature preparation logic as in prepare_features
                        target_cols = self.feature_metadata['target_columns']
                        feature_cols = [col for col in test_data.columns if col not in target_cols]
                        
                        # Create sample with the exact same columns as training
                        X_sample = test_data[feature_cols].head(3)
                        
                        # Verify the model can predict with this data
                        predictions_sample = self.best_model_info['model'].predict(X_sample)
                        ...
                        print(f"Created model signature with {len(feature_cols)} features")
                        
                    except Exception as e:
                        print(f"Could not create model signature: {e}")
                        print("Proceeding without signature and input example")
                    ...
                    # Log model performance metrics (convert to basic types)
                    metrics_to_log = {}
                    for key, value in self.best_model_info['metrics'].items():
                        metrics_to_log[key] = float(value)
                    
                    mlflow.log_metrics(metrics_to_log)
                    mlflow.log_param("model_type", str(self.best_model_info['name']))
                    ...
                    r2_score = self.best_model_info['metrics']['r2']
                    rmse_score = self.best_model_info['metrics']['rmse']
                    
                    print(f"Checking promotion criteria: R²={r2_score:.4f} > 0.7? {r2_score > 0.7}")
                    
                    if r2_score > 0.7:  # Threshold for staging
                        print(f"Model qualifies for Staging promotion")
                        # Note: Will promote after current run ends to avoid serialization issues
                    else:
                        print(f"Model performance below staging threshold (R²={r2_score:.4f} <= 0.7)")
                    
                    # Production comparison will be handled after run context ends
                    
                    # Log preprocessing artifacts for reproducibility
                    print("Logging preprocessing artifacts...")
                    try:
                        # Log feature metadata as artifact
                        with open('temp_feature_metadata.json', 'w') as f:
                            json.dump(self.feature_metadata, f, indent=2)
                        mlflow.log_artifact('temp_feature_metadata.json', 'preprocessing')
                        os.remove('temp_feature_metadata.json')
                        
                        # Log training configuration
                        training_config = {
                            'model_algorithm': self.best_model_info['name'],
                            'feature_count': len(feature_cols) if 'feature_cols' in locals() else 0,
                            'training_samples': X_sample.shape[0] if input_example is not None else 0,
                            'performance_threshold_staging': 0.7,
                            'performance_threshold_production': 0.95,
                            'experiment_name': EXPERIMENT_NAME
                        }
                        
                        with open('temp_training_config.json', 'w') as f:
                            json.dump(training_config, f, indent=2)
                        mlflow.log_artifact('temp_training_config.json', 'config')
                        os.remove('temp_training_config.json')
                        
                        print("Preprocessing artifacts logged")
                        
                    except Exception as e:
                        print(f"Warning: Could not log preprocessing artifacts: {e}")
            ...
            # Store promotion info for later (avoid serialization issues)
            self._store_promotion_info()
            
        else:
            print(f"\nBest model is baseline ({self.best_model_info['name']}) - no sklearn object to save")
    
    def _store_promotion_info(self):
        """Store promotion information for post-training promotion"""
        promotion_info = {
            'model_name': self.best_model_info['name'],
            'r2_score': float(self.best_model_info['metrics']['r2']),
            'rmse_score': float(self.best_model_info['metrics']['rmse']),
            'mae_score': float(self.best_model_info['metrics']['mae']),
            'mape_score': float(self.best_model_info['metrics']['mape']),
            'should_promote': float(self.best_model_info['metrics']['r2']) > 0.7
        }
        
        # Save to file for post-training promotion
        with open('temp_promotion_info.json', 'w') as f:
            json.dump(promotion_info, f, indent=2)
        
        if promotion_info['should_promote']:
            print(f"\nModel qualifies for promotion (R²={promotion_info['r2_score']:.4f} > 0.7)")
            print("Run promotion script: python promote_model.py")
        else:
            print(f"Model below promotion threshold (R²={promotion_info['r2_score']:.4f} <= 0.7)")
    
    def get_best_model_from_registry(self, stage: str = "Production") -> Any:
    ...
    def log_environment_info(self):
        """Log environment and dependency information for reproducibility"""
        try:
            import platform
            import sys
            
            # Try modern importlib.metadata first, fall back to pkg_resources
            requirements = []
            try:
                ...
                # Fall back to pkg_resources if importlib.metadata not available
                try:
                    ...
                    for package in installed_packages:
                        if package.project_name.lower() in key_packages:
            ...
            with open('temp_environment.json', 'w') as f:
            ...
            return 'temp_environment.json', 'temp_requirements.txt'
            
        except Exception as e:
            print(f"Warning: Could not log environment info: {e}")
            return None, None
    ...
    def run_training_pipeline(self):
        ...
        # 6. Save best model and log environment
        self.save_best_model()
        
        # 7. Log environment for reproducibility
        print("\nLogging environment information...")
        env_files = self.log_environment_info()
        if env_files and env_files[0]:
            try:
                with mlflow.start_run(run_name=f"Environment_Info_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                    mlflow.log_artifact(env_files[0], 'environment')
                    mlflow.log_artifact(env_files[1], 'environment')
                    mlflow.log_param("pipeline_completed", True)
                    mlflow.log_param("best_model", self.best_model_info['name'])
                    
                # Clean up temp files
                os.remove(env_files[0])
                os.remove(env_files[1])
                print("Environment information logged")
            except Exception as e:
                print(f"Warning: Could not log environment to MLflow: {e}")
        
        print("\nTraining pipeline completed successfully!")
        print("=" * 75)
        print(f"Best model: {self.best_model_info['name']}")
        print(f"Performance: RMSE={self.best_model_info['metrics']['rmse']:.2f}, R²={self.best_model_info['metrics']['r2']:.4f}")
